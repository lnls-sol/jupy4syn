...
# Logging function that write log information in a file and also in the notebook output cells
# (if this config is set True)
...
def configurate_motor(motor_pv_name='', motor_name='', config=Configuration()):
    ...
    try:
        ...
        logprint("Motor: \'" + motor_pv_name + "\' succesfully created as \'" + motor_name + "\'",
                 config=config)
    except Exception as error:
        logprint("Error trying to create Motor: \'" + motor_pv_name + "\' as \'" + motor_name + 
                 "\'", "[ERROR]", config=config)
        logprint(str(error), "[ERROR]", config=config)
